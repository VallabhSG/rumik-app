...
describe('UpdateBanner', () => {
  it('renders nothing when status is idle', () => {
    mockUseOtaUpdate.mockReturnValue({ status: 'idle', error: null, download: jest.fn(), applyNow: jest.fn() });
    const { toJSON } = render(<UpdateBanner />);
    expect(toJSON()).toBeNull();
  });

  it('renders nothing when up-to-date', () => {
    mockUseOtaUpdate.mockReturnValue({ status: 'up-to-date', error: null, download: jest.fn(), applyNow: jest.fn() });
    const { toJSON } = render(<UpdateBanner />);
    expect(toJSON()).toBeNull();
  });
  ...
  it('shows download button when available', () => {
    ...
    mockUseOtaUpdate.mockReturnValue({ status: 'available', error: null, download, applyNow: jest.fn() });
  ...
  it('shows spinner when downloading', () => {
    mockUseOtaUpdate.mockReturnValue({ status: 'downloading', error: null, download: jest.fn(), applyNow: jest.fn() });
    const { getByText } = render(<UpdateBanner />);
  ...
  it('shows restart button when ready', () => {
    ...
    mockUseOtaUpdate.mockReturnValue({ status: 'ready', error: null, download: jest.fn(), applyNow });
    const { getByText } = render(<UpdateBanner />);
